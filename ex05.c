...
int stopTime(double tic_in, double *toc_p)
{
   ...
   * write the results in `toc_p` */
    ...
    double elapseTime = MPI_Wtime() - tic_in;
    *toc_p = elapseTime;
  return 0;
}
    ...
    for (int numBytes = 8; numBytes <= maxSize; numBytes *= 8) {
      ...
      for (int t = 0; t < numTests; t++) {
        ...
        if (rank < numComm) {
           ...
           * receiving processes. */
            err = MPI_Bcast(buffer, numBytes, MPI_BYTE, 0, comm); MPI_CHK(err);
    ...
    for (int numBytes = 8; numBytes <= maxCollectiveSize; numBytes *= 8) {
      ...
      for (int t = 0; t < numTests; t++) {
        ...
        if (rank < numComm) {
           ...
           */
            err = MPI_Scatter(buffer,  numBytes, MPI_BYTE,
                              buffer2, numBytes, MPI_BYTE,
                              0, comm);MPI_CHK(err);
        }
        err = stopTime(tic, &tic); MPI_CHK(err);
        if (t) {
    ...
    for (int numBytes = 8; numBytes <= maxSize; numBytes *= 8) {
      ...
      for (int t = 0; t < numTests; t++) {
        ...
        if (rank < numComm) {
           ...
           * MPI_IN_PLACE).
           */
            err = MPI_Reduce(buffer2, buffer, numBytes*size, MPI_BYTE, MPI_SUM, 0, comm);MPI_CHK(err);
        }
        err = stopTime(tic, &tic); MPI_CHK(err);
        if (t) {
          timeAvg += tic;
    ...
    for (int numBytes = 8; numBytes <= maxCollectiveSize; numBytes *= 8) {
      ...
      for (int t = 0; t < numTests; t++) {
        ...
        if (rank < numComm) {
           ...
           * `buffer` on process 0.
           */
            err = MPI_Gather(buffer2, numBytes, MPI_BYTE,
                             buffer,  numBytes, MPI_BYTE,
                             0, comm);MPI_CHK(err);
        }
        err = stopTime(tic, &tic); MPI_CHK(err);
        if (t) {
    ...
    for (int numBytes = 8; numBytes <= maxSize; numBytes *= 8) {
      ...
      for (int t = 0; t < numTests; t++) {
        ...
        if (rank < numComm) {
           ...
           * `buffer` on process 0 (HINT: MPI_IN_PLACE again).
           */
            err = MPI_Allreduce(buffer2, buffer, numBytes*size, MPI_BYTE, MPI_SUM, comm);MPI_CHK(err);
        }
        err = stopTime(tic, &tic); MPI_CHK(err);
        if (t) {
          timeAvg += tic;
    ...
    for (int numBytes = 8; numBytes <= maxCollectiveSize; numBytes *= 8) {
      ...
      for (int t = 0; t < numTests; t++) {
        ...
        if (rank < numComm) {
           ...
           * `buffer`.
           */
            err = MPI_Allgather(buffer2, numBytes, MPI_BYTE,
                                buffer,  numBytes, MPI_BYTE,
                                comm);MPI_CHK(err);
        }
        err = stopTime(tic, &tic); MPI_CHK(err);
    ...
    for (int numBytes = 8; numBytes <= maxCollectiveSize; numBytes *= 8) {
      ...
      for (int t = 0; t < numTests; t++) {
        ...
        if (rank < numComm) {
           ...
           */
            err=MPI_ALLTOALL(buffer, numComm*numBytes, MPI_BYTE, buffer, numComm*numBytes, MPI_BYTE, comm);MPI_CHK(err);
        }
        err = stopTime(tic, &tic); MPI_CHK(err);
        if (t) {
          timeAvg += tic;
