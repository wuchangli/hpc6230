            ...
            for (int t = 0; t < numTests; t++) {
                ...
                if (rank < numComm) {
                    ...
                    if(rank == 0){
                        ...
                        err = MPI_Reduce(buffer, NULL, numBytes, MPI_CHAR, MPI_MIN, 0, subComm); MPI_CHK(err);
                    }
                }
                err = stopTime(tic, &tic); MPI_CHK(err);
                if (t) {
        ...
        for (int numBytes = 8; numBytes <= maxCollectiveSize; numBytes *= 8) {
            ...
            for (int t = 0; t < numTests; t++) {
                ...
                if (rank < numComm) {
                     ...
                     */
                    err = MPI_Alltoall(buffer, numBytes, MPI_CHAR, buffer, numBytes, MPI_CHAR, subComm);MPI_CHK(err);
                }
                err = stopTime(tic, &tic); MPI_CHK(err);
                if (t) {
                    timeAvg += tic;
