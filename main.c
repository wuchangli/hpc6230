...
int splitCommunicator(MPI_Comm comm, int firstCommSize, MPI_Comm *subComm_p)
{
    ...
    MPI_Comm_split(comm, color, rank, subComm_p);
        ...
        for (int numBytes = 8; numBytes <= maxSize; numBytes *= 8) {
            ...
            for (int t = 0; t < numTests; t++) {
                ...
                if (rank < numComm) {
                    ...
                    err = MPI_Reduce(buffer2, buffer, 8, MPI_CHAR, MPI_MIN, 0, subComm);MPI_CHK(err);
                }
                err = stopTime(tic, &tic); MPI_CHK(err);
                if (t) {
                    timeAvg += tic;
        ...
        for (int numBytes = 8; numBytes <= maxSize; numBytes *= 8) {
            ...
            for (int t = 0; t < numTests; t++) {
                ...
                if (rank < numComm) {
                     ...
                     */
                    err = MPI_Allreduce(buffer2, buffer, numBytes, MPI_CHAR, MPI_MIN, subComm);MPI_CHK(err);
                }
                err = stopTime(tic, &tic); MPI_CHK(err);
                if (t) {
                    timeAvg += tic;
        ...
        for (int numBytes = 8; numBytes <= maxCollectiveSize; numBytes *= 8) {
            ...
            for (int t = 0; t < numTests; t++) {
                ...
                if (rank < numComm) {
                     ...
                     * `buffer`.
                     */
                    err = MPI_Allgather(buffer2, 8, MPI_BYTE,
        ...
        for (int numBytes = 8; numBytes <= maxCollectiveSize; numBytes *= 8) {
            ...
            for (int t = 0; t < numTests; t++) {
                ...
                if (rank < numComm) {
                     ...
                     */
                    err = MPI_Alltoall(buffer, numComm*numBytes, MPI_BYTE, buffer, numComm*numBytes, MPI_BYTE, subComm);MPI_CHK(err);
                }
                err = stopTime(tic, &tic); MPI_CHK(err);
                if (t) {
                    timeAvg += tic;
